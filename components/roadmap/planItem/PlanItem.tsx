...
const PlanItem: FC<Props> = ({ title, skills, level, price, image }) => {
  return (
    <div className="flex flex-col gap-2  border border-neutral-200 rounded-[20px] shadow-[0_4px_24px_rgba(0,0,0,0.05)] ">
      <div className="flex gap-4 p-3">
        {/*<div className="min-w-[86px] h-[108px] relative rounded-md overflow-hidden">*/}
        {/*  <Image src={image || "/"} alt={title} layout={'fill'} objectFit={'cover'} />*/}
        {/*</div>*/}
        <div className="flex flex-col gap-3">
          <span className="xs:text-[14px] xl:text-[20px] font-[CodecPro-Bold] line-clamp-1 ">{title}</span>
          <div className="flex flex-row items-center gap-2 flex-wrap">
            <Badge type="secondary">{skills}</Badge>
