      ...
      <div className="flex flex-col justify-center items-center 2xl:max-w-[80%] xs:max-w-[90%] mb-12">
        ...
        <form className=" mx-auto w-full mt-[56px] ">
          ...
          {suggestions?.length > 0 && (
            <div className="max-h-60 overflow-auto shadow-[0_4px_18px_rgba(0,0,0,0.1)] rounded-[16px]  transform translate-y-[10px] z-10 border-t border-gray-600 w-full bg-white">
              <ul className="py-2  text-sm text-gray-700 divide-y divide-gray-100">
                {suggestions.map((item, index) => (
                  <li key={'SUGGESTION_ITEM_' + index} className=" py-2 px-4" onClick={() => {}}>
                    {item.title}
                  </li>
                ))}
              </ul>
